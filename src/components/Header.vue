...
<template>
    <header class="flex w-full bg-card h-18 items-center border-b border-border">
          ...
          <div class="hidden md:flex items-center justify-between w-full max-w-[1024px] mx-auto px-4 sm:px-8 py-10">
              <div>
                  <SidebarTrigger class="md:hidden" />
                  <h1 class="text-lg sm:text-2xl font-semibold">{{headerStore.pageTitle}}</h1>
                  <p class="hidden sm:inline">{{headerStore.pageDescription}}</p>
              </div>
              <Button size="lg" class="cursor-pointer" @click="headerStore.buttonAction?.onClick">
                  <Plus/>
                 <p class="hidden sm:inline">{{headerStore.buttonAction?.text}}</p> 
              </Button>
          </div>
          ...
          <div class="flex md:hidden items-center justify-between w-full px-4 py-4">
              ...
              <Button size="icon" class="cursor-pointer" @click="headerStore.buttonAction?.onClick">
                  <Plus/>
              </Button>
          </div>
